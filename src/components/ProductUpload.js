...
export default function ProductUpload({ setLoading }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${API_ENDPOINT}/product/upload`, formData, {
        headers: {
      ...
      if (error.response) {
        ...
        setError('Error updating products');
      }
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {success && <SuccessToast message="Products updated successfully!" />}
