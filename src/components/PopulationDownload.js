...
export default function PopulationDownload({ setLoading }) {
  ...
  const headers = [
    { label: "Zip", key: "Zip" },
    { label: "5 Mile Population", key: "5 Mile Population" },
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${API_ENDPOINT}/population/download`, formData, {
        headers: {
      ...
      const csv = [
        ...
        ...response.data.map(row =>
          headers.map(header => {
            return header.key === '5 Mile Population' ? `"${row[header.key]}"` : row[header.key]
          }).join(',')
        )
      ].join('\n');
      ...
      if (csv && csv.length > 0) {
        ...
        link.download = 'population_lookup_result.csv';
      ...
      if (error.response) {
        ...
        setError('Error looking up the populations.');
      }
      setLoading(false);
    }
  };
